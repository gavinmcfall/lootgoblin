...
export async function GET(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await (getDb() as any).select().from(schema.hoardLibraries).where(eq(schema.hoardLibraries.id, id));
  if (!row) return NextResponse.json({ error: 'not found' }, { status: 404 });
  return NextResponse.json({ hoardLibrary: row });
}
...
export async function PUT(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await (getDb() as any).update(schema.hoardLibraries).set({ ...body, updatedAt: new Date() }).where(eq(schema.hoardLibraries.id, id));
...
export async function DELETE(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!acl.allowed) return NextResponse.json({ error: acl.reason }, { status: user ? 403 : 401 });
  await (getDb() as any).delete(schema.hoardLibraries).where(eq(schema.hoardLibraries.id, id));
  return NextResponse.json({ ok: true });
}
