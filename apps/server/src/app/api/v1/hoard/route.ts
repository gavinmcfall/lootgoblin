...
export async function GET(req: Request) {
  ...
  const rows = await (getDb() as any).select().from(schema.hoardLibraries);
  return NextResponse.json({ hoardLibraries: rows });
}
...
export async function POST(req: Request) {
  ...
  const id = randomUUID();
  await (getDb() as any).insert(schema.hoardLibraries).values({ id, ...body });
