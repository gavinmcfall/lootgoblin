/**
 * Shared helpers for /api/v1/scout-auth/:sourceId/* routes — V2-003-T9.
 *
 * Centralises:
 ...
 * /api/v1/scout-credentials/[source]/route.ts coexists for cookie-jar
...
export async function upsertSourceCredential(args: {
  ...
  const existing = await db
    ...
    .where(eq(schema.scoutCredentials.scoutId, args.sourceId))
    .limit(1);
  ...
  if (existing[0]) {
    await db
      ...
      .where(eq(schema.scoutCredentials.id, existing[0].id));
...
> {
  ...
  const rows = await db
    .select({
      ...
      expiresAt: schema.scoutCredentials.expiresAt,
