/**
 * POST /api/v1/scout-auth/:sourceId/oauth/callback — V2-003-T9
 *
...
export async function POST(
  ...
  try {
    ...
    logger.warn({ err, scoutId: auth.sourceId }, 'source-auth/oauth/callback: token exchange fetch failed');
    return NextResponse.json(
  ...
  if (!tokenRes.ok) {
    ...
    logger.warn(
      { scoutId: auth.sourceId, upstreamStatus: tokenRes.status, upstreamBody },
