/**
 * POST /api/v1/scout-auth/:sourceId/refresh — V2-003-T9
 *
...
export async function POST(
  ...
  try {
    ...
    logger.warn({ err, scoutId: auth.sourceId }, 'source-auth/refresh: token endpoint unreachable');
    return NextResponse.json(
  ...
  return NextResponse.json({ ok: true, scoutId: auth.sourceId, expiresAt: expiresAtMs });
}
