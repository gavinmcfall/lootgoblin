/**
 * GET + DELETE /api/v1/scout-auth/:scoutId — V2-003-T9
 *
 * GET    — read current credential status. NEVER returns the secret payload.
 * DELETE — revoke (remove the scout_credentials row).
 *
 ...
 *
 * Coexists with the legacy /api/v1/scout-credentials/[source]/route.ts
...
export async function GET(
  req: NextRequest,
  context: { params: Promise<{ scoutId: string }> },
) {
  const { scoutId: rawId } = await context.params;
  const auth = await authorizeRead(req, rawId);
  if (!auth.ok) return auth.response;

  const status = await readCredentialStatus(auth.sourceId);
  return NextResponse.json({ scoutId: auth.sourceId, ...status });
}
...
export async function DELETE(
  ...
  return NextResponse.json({ ok: true, scoutId: auth.sourceId, removed });
}
