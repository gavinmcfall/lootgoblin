...
export async function PUT(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { hoardId, credentialId } = (await req.json()) as { hoardId?: string; credentialId?: string };
  await (getDb() as any).update(schema.items).set({ hoardId, credentialId, updatedAt: new Date() }).where(eq(schema.items.id, id));
