/**
 * adapter.ts — V2-005d-d T_dd1
 *
 * OctoPrint HTTP-multipart upload DispatchHandler.
 *
 * Flow:
 *   1. Parse `printer.connectionConfig` via OctoprintConnectionConfig.
 ...
 *      OctoprintCredentialPayload. Wrong shape:
 *        - requiresAuth=true → 'auth-failed' (we can't authenticate)
 *        - requiresAuth=false → log warn and proceed without header
 *   4. POST multipart/form-data to {scheme}://{host}:{port}{apiPath}/files/local
 *      with fields: file, select=config.select, print=config.startPrint.
 *      Header: X-Api-Key when apiKey is available.
 *   5. Map response to DispatchOutcome:
 *        - 200/201   → success (remoteFilename from result.files.local.path or filename)
 *        - 401/403   → 'auth-failed'
 *        - other 4xx → 'rejected'
 *        - 5xx       → 'unknown'
 ...
 *   https://docs.octoprint.org/en/master/api/files.html
 */
...
import { OctoprintConnectionConfig, OctoprintCredentialPayload } from './types';

export const OCTOPRINT_KIND = 'fdm_octoprint' as const;
export const OCTOPRINT_TIMEOUT_MS = 60_000;
...
export function createOctoprintHandler(opts?: {
  ...
  const timeoutMs = opts?.timeoutMs ?? OCTOPRINT_TIMEOUT_MS;
  ...
  return {
    ...
    async dispatch(ctx: DispatchContext): Promise<DispatchOutcome> {
      ...
      const parsedConfig = OctoprintConnectionConfig.safeParse(ctx.printer.connectionConfig);
      ...
      if (ctx.credential !== null) {
        const parsedCred = OctoprintCredentialPayload.safeParse(ctx.credential.payload);
        if (parsedCred.success) {
          ...
          ctx.logger.warn(
            ...
            'octoprint: credential payload invalid but requiresAuth=false — proceeding without X-Api-Key',
          );
        }
      }

      // 4. Build URL. apiPath is concatenated directly (e.g. '/api' or '/octoprint/api').
      const url = `${config.scheme}://${config.host}:${config.port}${config.apiPath}/files/local`;
      ...
      fd.append('select', config.select ? 'true' : 'false');
      ...
      try {
        ...
        ctx.logger.warn(
          ...
          'octoprint: dispatch failed with unexpected error',
        );
        return {
          kind: 'failure',
          reason: 'unknown',
      ...
      if (status === 200 || status === 201) {
        ...
        try {
          ...
          const localPath = (body as { files?: { local?: { path?: unknown } } })?.files?.local?.path;
        ...
        try {
          ...
          ctx.logger.warn(
            ...
            'octoprint: touchLastUsed failed — non-fatal',
          );
        }
      ...
      if (status === 401 || status === 403) {
        ctx.logger.warn(
          ...
          'octoprint: auth rejected by printer',
        );
      ...
      if (status >= 400 && status < 500) {
        ctx.logger.warn(
          ...
          'octoprint: printer rejected upload',
        );
      ...
      ctx.logger.warn(
        ...
        'octoprint: printer returned server error',
      );
