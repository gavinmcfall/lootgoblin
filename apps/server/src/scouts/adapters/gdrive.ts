/**
 * gdrive.ts — Google Drive ScoutAdapter (V2-003-T8)
 *
...
export function createGdriveAdapter(
...
): ScoutAdapter & SubscribableAdapter {
  ...
  return {
    ...
    fetch(context: FetchContext, target: FetchTarget): AsyncIterable<ScoutEvent> {
      return {
...
async function* maybeRefreshToken(
  ...
  tokenEndpoint: string,
  httpFetch: typeof fetch,
): AsyncGenerator<ScoutEvent, GDriveCredentials | false | 'failed', void> {
  const oauthBag =
    creds.kind === 'oauth'
      ? creds
...
async function* fetchMetadata(
  ...
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, GDriveFile | null, void> {
...
async function* enumerateFolder(
  ...
  signal: AbortSignal | undefined,
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, { files: GDriveFile[]; skipped: SkipRecord[] }, void> {
...
async function* handleFolder(
  ...
  context: FetchContext,
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, void, void> {
...
async function* downloadFile(
  ...
  context: FetchContext,
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, NormalizedItem['files'][number] | null, void> {
  ...
  ): Promise<{ outcome: 'value'; res: Response } | { outcome: 'auth-rejected' } | { outcome: 'terminal'; failedEvent: ScoutEvent }> {
...
): AsyncGenerator<ScoutEvent, RequestOutcome<T>, void> {
 ...
 *
 * Discovery-side flavour — yields `DiscoveryEvent` (not `ScoutEvent`).
 */
