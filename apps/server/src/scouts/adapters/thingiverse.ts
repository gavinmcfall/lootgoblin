/**
 * thingiverse.ts — Thingiverse ScoutAdapter (V2-003b-T1)
 *
...
async function* maybeRefreshToken(
  ...
  httpFetch: typeof fetch,
): AsyncGenerator<ScoutEvent, ThingiverseCredentials | false | 'failed', void> {
  const oauthBag =
    creds.kind === 'oauth'
      ? creds
...
async function* fetchThingMetadata(
  ...
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, ThingMetadata | null, void> {
...
async function* fetchThingFiles(
  ...
  signal: AbortSignal | undefined,
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, ThingFile[] | null, void> {
...
async function* fetchAncestors(
  ...
  signal: AbortSignal | undefined,
  env: RequestEnv,
): AsyncGenerator<ScoutEvent, ThingAncestor[] | null, void> {
...
): AsyncGenerator<ScoutEvent, T | null, void> {
 ...
 * ScoutEvent. Mirrors `maybeRefreshToken` (fetch-side) logic.
 */
