 ...
 * Adapters are registered in createDefaultScoutRegistry (wired below).
 */
...
export type { ScoutRegistry } from './registry';
...
import type { ScoutRegistry } from './registry';

/**
 * Create a ScoutRegistry pre-populated with all currently implemented
 ...
 */
export function createDefaultScoutRegistry(): ScoutRegistry {
  const registry = createRegistry();
  registry.register(createUploadAdapter());

  // V2-004 T7: adapters that satisfy BOTH ScoutAdapter (ingest) AND
 ...
 *
 * Routes that need adapter metadata (e.g. /api/v1/scouts, /api/v1/ingest,
 * /api/v1/scout-auth/:sourceId) import this singleton instead of calling
 * createDefaultScoutRegistry() per request. The factory is cheap, but the
 ...
 */
export const defaultRegistry: ScoutRegistry = createDefaultScoutRegistry();
