 ...
 *
 * Consumes an AsyncIterable<ScoutEvent> from any ScoutAdapter.
...
import type { ScoutAdapter, FetchTarget, NormalizedItem } from './types';
          ...
          onTokenRefreshed: async (newCredentials: Record<string, unknown>) => {
            try {
              ...
              const rows = await db()
                ...
                .where(eq(schema.scoutCredentials.scoutId, adapter.id))
                .limit(1);
              const row = rows[0];
              if (!row) {
                logger.warn(
