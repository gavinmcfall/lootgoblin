 ...
 *
 * Maintains the set of registered ScoutAdapters and routes URLs + ids
...
import type { ScoutAdapter, SourceId } from './types';
...
export interface ScoutRegistry {
   ...
   */
  register(adapter: ScoutAdapter): void;
   ...
   */
  resolveUrl(url: string): ScoutAdapter | null;

  /**
   * Return the adapter registered under the given id, or null if not found.
   */
  getById(id: SourceId): ScoutAdapter | null;
...
/**
 * Create a new in-memory ScoutRegistry instance.
 *
 * Adapter ids are stored in insertion order. `list()` preserves that order.
 */
export function createRegistry(): ScoutRegistry {
  // Map preserves insertion order — `list()` can use Map iteration directly.
  const adapters = new Map<SourceId, ScoutAdapter>();
  ...
  return {
    register(adapter: ScoutAdapter): void {
      if (adapters.has(adapter.id)) {
        logger.warn(
          { sourceId: adapter.id },
          'ScoutRegistry: duplicate registration for sourceId — replacing existing adapter',
        );
      }
    ...
    resolveUrl(url: string): ScoutAdapter | null {
      let first: ScoutAdapter | null = null;
      let matchCount = 0;
        ...
        if (adapter.supports(url)) {
          ...
          if (first === null) {
            ...
            logger.warn(
              ...
              'ScoutRegistry: multiple adapters claim the same URL — first-registered wins',
            );
    ...
    registerSubscribable(adapter: SubscribableAdapter): void {
      if (subscribableAdapters.has(adapter.id)) {
        logger.warn(
          { sourceId: adapter.id },
          'ScoutRegistry: duplicate subscribable registration for sourceId — replacing existing adapter',
        );
      }
