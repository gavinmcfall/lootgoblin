 ...
 * -------------------------------------
 * The existing `ScoutAdapter` is the ingest-side contract: given a target
 ...
 *
 * Relationship to `ScoutAdapter`
 * ----------------------------------
 * One concrete adapter (e.g. `createMakerWorldAdapter`) MAY implement BOTH
 * interfaces — the same factory exports a value that is both `ScoutAdapter`
 ...
 * via the regular `ScoutAdapter.fetch`.
 *
...
export type DiscoveryContext = {
   ...
   * same contract as `ScoutAdapter.fetch`'s `onTokenRefreshed`. Adapters
...
export interface SubscribableAdapter {
  /** Must match a `SourceId` value — same as the corresponding `ScoutAdapter`. */
