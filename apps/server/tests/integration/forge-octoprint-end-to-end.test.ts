/**
 * End-to-end integration test — V2-005d-d T_dd2.
 *
 * Mirrors `forge-moonraker-end-to-end.test.ts` (V2-005d-a T_da7) but exercises
 * the OctoPrint dispatcher instead. The full chain:
 *
 *   1. POST /api/v1/forge/printers/:id/credentials → AES-GCM encrypt + store
 *      (kind='octoprint_api_key', payload={apiKey})
 *   2. Worker reads dispatch_jobs WHERE status='claimable'
 *   3. Worker resolves DispatchHandlerRegistry → OctoPrint adapter
 *   4. Adapter decrypts credential + builds multipart upload
 *   5. msw intercepts http://test-printer:80/api/files/local and asserts the
 *      X-Api-Key header + form fields (select=true, print=true) are correct
 ...
 *      Per FD-L4: state STOPS at 'dispatched' — completedAt remains null.
 *
 * Adapter unit/edge cases live in `forge-octoprint-adapter.test.ts`. This file
 * verifies the full V2-005d-d wiring across all primitives at once.
 */
import {
  describe,
  it,
...
import { createOctoprintHandler } from '../../src/forge/dispatch/octoprint/adapter';
...
const DB_PATH = '/tmp/lootgoblin-forge-octoprint-e2e.db';
const DB_URL = `file:${DB_PATH}`;
const TEST_SECRET = 'x'.repeat(32);
const TEST_API_KEY = 'e2e-test-key-67890';
const PRINTER_HOST = 'test-printer';
const PRINTER_PORT = 80;
// Note: undici/fetch normalizes the default :80 port out of http URLs before
// dispatching, so msw sees the URL without the explicit port. Register the
// handler against the normalized form.
const UPLOAD_URL = `http://${PRINTER_HOST}/api/files/local`;
...
async function seedUser(): Promise<string> {
  ...
  await db().insert(schema.user).values({
    id,
    name: 'OctoPrint E2E User',
    email: `${id}@octoprint-e2e.test`,
    emailVerified: false,
    createdAt: new Date(),
    updatedAt: new Date(),
  });
...
describe('OctoPrint dispatch end-to-end — V2-005d-d T_dd2', () => {
  it('claimable job → registry → adapter → msw upload → dispatched + last_used_at bumped', async () => {
    ...
    const tmpdir = await fsp.mkdtemp(path.join(os.tmpdir(), 'octoprint-e2e-'));
    const gcodePath = path.join(tmpdir, 'cube.gcode');
    const gcodeContent = '; lootgoblin V2-005d-d end-to-end\nG28\nM84\n';
    ...
    // 6. Register the REAL OctoPrint handler.
    getDefaultRegistry().register(createOctoprintHandler());

    // 7. Wire msw to mock OctoPrint. Capture the request for assertions.
    let hits = 0;
    server.use(
        ...
        return HttpResponse.json(
          {
            ...
            files: {
              local: { path: 'lootgoblin/cube.gcode', name: 'cube.gcode' },
            },
          },
          { status: 201 },
        );
    ...
    // after a successful upload). Per FD-L4: state stops at 'dispatched';
    // dispatched → completed is closed by V2-005f via real printer status
    // events. "We sent the file" ≠ "the print finished".
    const rows = await db()
      .select()
    ...
    expect(captured.formFields.select).toBe('true');
